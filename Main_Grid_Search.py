...
initial_weight_per_word = [0.5, 1, 2]
rewards = [0.5, 1, 2]
discounts = [0.95,0.99]
runs = 5
...
for reward in rewards:
    for discount in discounts:
        for initial_weight in initial_weight_per_word:
            for run in range(runs):
                ...
                wins_in_last_250 = 0
                wins_this_run = 0
                agent = Agent(list_of_words,discount,reward)  # initialize agent with wordlist, discount factor and reward value
                ...
                agent.setup_weights(initial_weight)  # set up agent's weights
                analytics = [[], []]
                ...
                for epoch in range(epochs+1):  # for each game
                    ...
                    if epoch % 250 == 0 and epoch != 0:  # print status each 50th epoch
                        ...
                        gridsearch["d_"+str(discount)+"|_r_"+str(reward)+"|iw_"+str(initial_weight)] = wins_in_last_250/total_games
                    ...
                    for rounds in range(6):
                        if not won:
                            ...
                            if won:
                                wins_in_last_250 += 1
                end_time = time.time()
file = open("gridsearch-results.txt","a")
file.write(str(gridsearch))
